...
收到需要拆解的文本后，**第一步**先判断输入类型，再判断长度。路径优先于长度判断。

| 条件 | 路径 |
|------|------|
| 输入是文件路径（以 `/` `~` `./` `../` 开头，或包含 `.md` 等扩展名且无连续段落文本特征） | → 移交给 `aolun-fileflow` |
| 输入是目录路径（以 `/` 结尾或路径指向目录） | → 移交给 `aolun-fileflow` |
| 输入文本字符数 ≥ 1500 | → 移交给 `aolun-fileflow`（文件持久化模式） |
| 输入文本字符数 < 1500 | → 进入 `## 意图问询`（见下方） |

> 路径优先于长度判断。文件路径通常短于 1500 字符，如果不单独检测，路径输入永远不会触发 fileflow。

> 长度判断方法：目测或计数均可。中文500字约等于1500字符，英文250词约等于1500字符，作为参考基准。
...
> 仅在输入字符数 < 1500 且不是路径时执行此步骤。路径输入和长文本已由上一步强制移交给 `aolun-fileflow`，不走此流程。
>
> **路径输入的路由：** 当输入被识别为文件路径或目录路径时，直接移交 fileflow，跳过意图问询。用户提供了路径就意味着他们要分析这个文件/目录，不需要再问目标。
