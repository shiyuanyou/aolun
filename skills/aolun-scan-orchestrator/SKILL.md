...
> - **> 8000 token**：主 agent 按该扫描器的任务类型，从原文中**机械摘取**相关段落（直接复制原句，不改写），填入 `[原文]` 字段，并在字段头部注明 `（节选，原文未改写）`。
>   - scan-logic 取：含因果词的句子、结论性段落、核心论点段落
>   - scan-engineering：技术参数、接口定义、依赖声明、数字指标段落
>   - scan-history：历史对比、先例描述、"曾经/prior work"类段落
>   - scan-motive：作者立场声明、利益相关方描述、强调词密集段落
...
原文：
[粘贴原文——完整原文或按上方规则机械节选的原文段落，均为原文措辞，未经改写]
...
弱点清单（最多5条，按攻击力排序）：
  1. [谬误类型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  2. [谬误类型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  3. ...
```
...
弱点清单（最多5条，按攻击力排序）：
  1. [弱点类型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  2. [弱点类型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  3. ...
```
...
弱点清单（最多5条，按攻击力排序）：
  1. [历史原型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  2. [历史原型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  3. ...
```
...
弱点清单（最多5条，按攻击力排序）：
  1. [动机扭曲类型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  2. [动机扭曲类型] — [原文证据，直接引用原句] — 攻击力：[高/中/低]
  3. ...
```
