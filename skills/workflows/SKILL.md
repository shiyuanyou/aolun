...
name: workflows
description: |
  触发：当你需要对一篇帖子执行完整的拆解攻击流程时调用；提供三条标准化工作流，定义 skill 的调用顺序、步骤间的数据传递和终止条件。
  English: Trigger when executing a full dissect-and-attack pipeline on a target post. Provides three standardized workflows with defined skill sequencing, data handoff, and termination conditions.
---
...
```
dissector-concept → scanner-logic → attack-writer（快速评论模式）
   概念解剖      →   逻辑扫描   →   输出50-200字攻击文
```

### 步骤详解

**Step 1：dissector-concept（概念层解剖）**
- 目标：30秒内找到最明显的概念问题
- 传递给 Step 2：
  ```
  核心声称：[一句话]
...
dissector-concept → dissector-mechanism → dissector-constraint → dissector-interest
     概念          →      机制           →       约束          →      利益
         ↓
scanner-logic + scanner-engineering + scanner-history + scanner-motive（并行）
   逻辑扫描  +    工程扫描         +   历史扫描     +   动机扫描
         ↓
   other-mountains（可选，如果有值得引入的跨领域解法）
         ↓
   attack-writer（标准攻击文模式）
```
...
**Phase 4 终止条件：**
文章通过质量自检清单（见 attack-writer 的质量自检部分）。
...
| 只需要找跨领域解法，不需要攻击文 | 四层解剖 + other-mountains |
| 只需要确认一个逻辑是否成立 | dissector-concept + scanner-logic |
