...
name: arming-liao
description: |
  触发：每次新的顶层对话开始时自动调用，建立"拆底朝天、立而破之"的总原则，并为后续任务路由下游 skill。如果你是被派遣执行单一具体任务的子 agent，则跳过此 skill。
  English: Trigger at the start of each new top-level conversation to establish the core posture of "tear it apart, then build something better." Skip this skill when you are a delegated sub-agent handling a narrow, concrete task.
---
...
```
arming-liao（路由）
    → dissector-concept   概念层解剖
    → dissector-mechanism 机制层解剖
    → dissector-constraint 约束层解剖
    → dissector-interest  利益层解剖
    → scanner-logic       逻辑弱点扫描
    → scanner-engineering 工程弱点扫描
    → scanner-history     历史弱点扫描
    → scanner-motive      动机弱点扫描
    → other-mountains     跨领域解法引擎
    → attack-writer       攻击文生成器
```
...
| 你的目标 | 最少需要的路径 |
|---------|------------|
| 快速摸底一篇帖子 | dissector-concept + scanner-logic |
| 写一篇有力的技术评论 | 四层解剖 + 全部扫描器 + attack-writer |
| 找到真正的跨领域解法 | 四层解剖 + other-mountains |
| 完整拆底朝天 | 全路径 |
